...
//Code Here
const each = (arr, cb) => arr.map((name, i) => cb(name, i));
...
// const getUserById = (arr, id, cb) => arr.forEach((el, idx) => el === id? cb(idx))
...
const evens = (arr) => arr.filter(num => {
  if (num % 2 === 0) {
    return num;
  }
})
...
// REPLACE THIS WITH YOUR CODE
const startWithLetterA = (arr) => arr.filter(letter => {
...
// REPLACE THIS WITH YOUR CODE
const sortNumbers = (arr) => arr.sort((a, b) => a > b ? 1 : -1)
...
// REPLACE THIS WITH YOUR CODE
const findProgrammer = (arr) => arr.filter(employee => employee.job === 'programmer')[0]
...
let orderTotals = orders.map(bill => {
  let tax = bill.tax + 1;
  return bill.price * tax;
}) ; // Code here
...
// REPLACE THIS WITH YOUR CODE
const sortMenuItems = (arr) => arr.sort((a, b) => a.price - b.price )
...
// REPLACE THIS WITH YOUR CODE
const productOfArray = (arr) => arr.reduce((total, num) => total * num, 1) 
